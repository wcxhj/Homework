...
class TextSplitter():
    ...
    def _split_page(self, page: Dict[str, any], chunk_size: int = 300, chunk_overlap: int = 50) -> List[Dict[str, any]]:
        ...
        分块策略：
        1. 使用RecursiveCharacterTextSplitter按字符递归分割
        2. 模型：gpt-4o (使用对应的tiktoken编码器)
        3. 分割长度：默认300个token
        4. 重叠长度：默认50个token (用于保持上下文连续性)
        5. 分割优先级：["\n\n", "\n", " ", ""] (按换行、空格、字符分割)
        """
    ...
    def split_markdown_file(self, md_path: Path, chunk_size: int = 30, chunk_overlap: int = 5):
        ...
        这种分块方式适用于按行组织的markdown内容，如表格或列表。
